...
function App() {
  ...
  const contractAddress = "0x83B4c81d6A6cF62C185eA0F1cCb6a70251104e13";
  ...
  const checkIfWalletIsConnected = async () => {
    try {
      ...
      if (window.ethereum) {
        ...
        console.log("No Metamsk detected");
      }
    } catch (error) {
      console.log(error);
    }
    ...
    try {
      ...
      if (window.ethereum) {
        ...
        if (owner.toLowerCase() === account.toLowerCase()) {
          ...
          setError("Please install a Metamask waller to use our bank");
        }
      }
    } catch (error) {
      console.log(error);
  ...
  const customerBalanceHandler = async () => {
    try {
      ...
      if (window.ethereum) {
        ...
        console.log("Retrieved banalve...", balance);
      } else {
        console.log("Ethereum object not found, install metamask");
        setError("Please install a metamask wallet to use our bank");
      }
  ...
  const deposityMoneyHandler = async (event) => {
    try {
      ...
      if (window.ethereum) {
        ...
        const txn = bankContract.depositMoney({
        ...
        console.log("Ethereum object not found, install metamask.");
        setError("Please install metamask wallet to us our bank");
      }
    } catch (error) {
      console.log(error);
    }
